...
class TransactionUtil extends Util
{
    ...
    public function getTransactionTotals(
        ...
        if (in_array('purchase_damage_return', $transaction_types)) {
            $query->addSelect(
                DB::raw("SUM(IF(transactions.type='purchase_damage_return', final_total, 0)) as total_purchase_damage_return_inc_tax"),
                DB::raw("SUM(IF(transactions.type='purchase_damage_return', total_before_tax, 0)) as total_purchase_damage_return_exc_tax")
            );
        }
        ...
        if (in_array('purchase_damage_return', $transaction_types)) {
            $output['total_purchase_damage_return_inc_tax'] = !empty($transaction_totals->total_purchase_damage_return_inc_tax) ?
                $transaction_totals->total_purchase_damage_return_inc_tax : 0;

            $output['total_purchase_damage_return_exc_tax'] =
                !empty($transaction_totals->total_purchase_damage_return_exc_tax) ?
                $transaction_totals->total_purchase_damage_return_exc_tax : 0;
        }
    ...
    public function getListPurchases($business_id)
    {
        $purchases = Transaction::leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
            ...
            ->leftjoin('transactions AS DR', function ($join) {
                $join->on('transactions.id', '=', 'DR.return_parent_id')
                    ->where('DR.type', 'purchase_damage_return');
            })
            ...
            ->select(
                ...
                'DR.id as damage_return_transaction_id',
                ...
                DB::raw('COUNT(DR.id) as damage_exists'),
                DB::raw('COALESCE(DR.final_total, 0) as amount_damage'),
