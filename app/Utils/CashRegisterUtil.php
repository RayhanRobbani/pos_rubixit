...
class CashRegisterUtil extends Util
{
    ...
    public function getRegisterDetails($register_id = null)
    {
        ...
        // CUSTOM
        // TO SHOW TOTAL BUSINESS EXPENSES IN CASH REGISTER
        $query_transaction = Transaction::where('transactions.business_id', auth()->user()->business_id)->where('created_at', '>=', $register_details['open_time']);
        $total_expense = $query_transaction->addSelect(
            DB::raw("SUM(IF(transactions.type='expense', final_total, 0)) as total_expense")
        )->first();
        $register_details['total_expense'] = $total_expense->total_expense;

        $total_sell_return = $query_transaction->addSelect(
            DB::raw("SUM(IF(transactions.type='sell_return', final_total, 0)) as total_sell_return"),
        )->first();
        $register_details['total_sell_return'] = $total_sell_return->total_sell_return;
        // CUSTOM
