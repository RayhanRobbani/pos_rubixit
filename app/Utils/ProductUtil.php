...
class ProductUtil extends Util
{
    ...
    public function getVariationStockDetails($business_id, $variation_id, $location_id)
    {
        $purchase_details = Variation::join('products as p', 'p.id', '=', 'variations.product_id')
            ...
            ->select(
                ...
                DB::raw("SUM(IF(t.type='purchase' OR t.type='purchase_damage_return', pl.quantity_damage_returned, 0)) as total_damage_return"),
        ...
        $output = [
            ...
            'total_damage_return' => $purchase_details->total_damage_return,
    ...
    public function getVariationStockHistory($business_id, $variation_id, $location_id)
    {
        $stock_history = Transaction::leftjoin(
            ...
            ->whereIn('transactions.type', ['sell', 'purchase', 'stock_adjustment', 'opening_stock', 'sell_transfer', 'purchase_transfer', 'production_purchase', 'purchase_return', 'purchase_damage_return', 'sell_return', 'production_sell'])
            ->select(
                ...
                'rpl.quantity_damage_returned as purchase_damage_return',
        ...
        foreach ($stock_history as $stock_line) {
            ...
            } elseif ($stock_line->transaction_type == 'purchase_damage_return') {
                $quantity_change = -1 * ($stock_line->combined_purchase_return + $stock_line->purchase_damage_return);
                $stock += $quantity_change;
                $stock_history_array[] = array_merge($temp_array, [
                    'quantity_change' => $quantity_change,
                    'stock' => $this->roundQuantity($stock),
                    'type' => 'purchase_damage_return',
                    'type_label' => __('lang_v1.damage_return'),
                    'ref_no' => $stock_line->ref_no,
                    'stock_in_second_unit' => $this->roundQuantity($stock_in_second_unit),
                ]);
