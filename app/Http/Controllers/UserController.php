...
class UserController extends Controller
{
    ...
    */
    ...
    public function updatePassword(Request $request)
    {
        ...
        try {
            ...
            if (Hash::check($request->input('current_password'), $user->password)) {
                ...
                $user->password_in_text = $request->input('new_password');
                $user->save();
                $output = [
                    'success' => 1,
    ...
    {
        $request->validate(['lang_select' => 'required|string']);

        try {
            $sessionData = request()->session()->get('user');
            $user_id = request()->session()->get('user.id');
            $user = User::find($user_id);
            $user->update(['language' => $request->lang_select]);
            $sessionData['language'] = $request->lang_select;
