...
class ReportController extends Controller
{
    ...
    public function getPurchaseSell(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $transaction_types = [
                'purchase_return', 'purchase_damage_return', 'sell_return',
            ];
            ...
            $total_purchase_damage_return_inc_tax = $transaction_totals['total_purchase_damage_return_inc_tax'];
            $total_sell_return_inc_tax = $transaction_totals['total_sell_return_inc_tax'];

            $difference = [
                'total' => $sell_details['total_sell_inc_tax'] - $total_sell_return_inc_tax - ($purchase_details['total_purchase_inc_tax'] - $total_purchase_return_inc_tax - $total_purchase_damage_return_inc_tax),
            ...
            return [
                ...
                'total_damage_return' => $total_purchase_damage_return_inc_tax,
    ...
    /**
     * Shows product damage report
     *
     * @return \Illuminate\Http\Response
     */
    public function getproductDamageReport(Request $request)
    {
        if (!auth()->user()->can('purchase_n_sell_report.view')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        if ($request->ajax()) {
            ...
            $query = PurchaseLine::join(
                ...
                ->where('t.business_id', $business_id)
                ->where('t.type', 'purchase')
                ->where('purchase_lines.quantity_damage_returned', '!=', 0)
                ->select(
                    'p.name as product_name',
                    'p.type as product_type',
                    'pv.name as product_variation',
                    ...
                    't.transaction_date as transaction_date',
                    'purchase_lines.purchase_price_inc_tax as unit_purchase_price',
                    DB::raw('purchase_lines.quantity_damage_returned as damage_qty'),
                    'purchase_lines.quantity_adjusted',
                    'u.short_name as unit',
                    DB::raw('(purchase_lines.quantity_damage_returned * purchase_lines.purchase_price_inc_tax) as subtotal')
                )
                ->groupBy('purchase_lines.id');
            if (!empty($variation_id)) {
                $query->where('purchase_lines.variation_id', $variation_id);
            ...
            return Datatables::of($query)
                ...
                ->editColumn('damage_qty', function ($row) {
                    return '<span data-is_quantity="true" class="display_currency damage_qty" data-currency_symbol=false data-orig-value="' . (float) $row->damage_qty . '" data-unit="' . $row->unit . '" >' . (float) $row->damage_qty . '</span> ' . $row->unit;
                })
                ->editColumn('subtotal', function ($row) {
                    return '<span class="row_subtotal"  
                     data-orig-value="' . $row->subtotal . '">' .
                ...
                ->editColumn('supplier', '@if(!empty($supplier_business_name)) {{$supplier_business_name}},<br>@endif {{$supplier}}')
                ->rawColumns(['ref_no', 'unit_purchase_price', 'subtotal', 'damage_qty', 'supplier'])
                ->make(true);
        }
        ...
        return view('report.product_damage_report')
            ->with(compact('business_locations', 'suppliers', 'brands'));
    }
