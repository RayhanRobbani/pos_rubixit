...
class PurchaseReturnController extends Controller
{
    ...
    public function index_damage()
    {
        ...
        if (request()->ajax()) {
            $purchases_returns = Transaction::leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
                ...
                ->where('transactions.business_id', $business_id)
                ->where('transactions.type', 'purchase_damage_return')
                ->select(
                    'transactions.id',
                    'transactions.transaction_date',
                    'transactions.ref_no',
                ...
                ->addColumn('action', function ($row) {
                    $html = '<div class="btn-group">
                        ...
                        '<span class="caret"></span><span class="sr-only">Toggle Dropdown
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-right" role="menu">';
                    if (!empty($row->return_parent_id)) {
                        $html .= '<li><a href="' . action([\App\Http\Controllers\PurchaseReturnController::class, 'add_damage'], $row->return_parent_id) . '" ><i class="glyphicon glyphicon-edit"></i>' .
                            __('messages.edit') .
                            '</a></li>';
                    }
                    // else {
                    ...
                    $html .= '<li><a href="' . action([\App\Http\Controllers\PurchaseReturnController::class, 'destroy_damage'], $row->id) . '" class="delete_purchase_return" ><i class="fa fa-trash"></i>' .
                        __('messages.delete') .
                        '</a></li>';
                    $html .= '</ul></div>';
                ...
                ->setRowAttr([
                    'data-href' => function ($row) {
                        if (auth()->user()->can('purchase.view')) {
                            ...
                            return  action([\App\Http\Controllers\PurchaseReturnController::class, 'show_damage'], [$return_id]);
                        } else {
                            return '';
                        }
                    },
        ...
        return view('purchase_return.index_damage')->with(compact('business_locations', 'suppliers'));
    }
    ...
    public function add($id)
    {
        ...
        $purchase = Transaction::where('business_id', $business_id)
            ->where('type', 'purchase')
            ->with(['purchase_lines', 'contact', 'tax', 'return_parent' => function (Builder $query) {
                $query->where('type', 'purchase_return');
            }, 'purchase_lines.sub_unit', 'purchase_lines.product', 'purchase_lines.product.unit'])
            ->find($id);
        ...
        foreach ($purchase->purchase_lines as $key => $value) {
            $qty_available = $value->quantity - $value->quantity_sold - $value->quantity_adjusted - $value->quantity_damage_returned;
    ...
    /**
     * Show the form for purchase damage return.
     *
     * @return \Illuminate\Http\Response
     */
    public function add_damage($id)
    {
        if (!auth()->user()->can('purchase.update')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        $purchase = Transaction::where('business_id', $business_id)
            ->where('type', 'purchase')
            ->with(['purchase_lines', 'contact', 'tax', 'return_parent' => function (Builder $query) {
                $query->where('type', 'purchase_damage_return');
            }, 'purchase_lines.sub_unit', 'purchase_lines.product', 'purchase_lines.product.unit'])
            ->find($id);
        ...
        return view('purchase_return.add_damage')
            ->with(compact('purchase'));
    }
    ...
    /**
     * Saves Purchase damage returns in the database.
     *
     * @param  \Illuminate\Http\Request  $request
     * @return \Illuminate\Http\Response
     */
    public function store_damage(Request $request)
    {
        ...
        try {
            ...
            foreach ($purchase->purchase_lines as $purchase_line) {
                $old_return_qty = $purchase_line->quantity_damage_returned;
                ...
                $purchase_line->quantity_damage_returned = $return_quantity;
                $purchase_line->save();
                $return_total += $purchase_line->purchase_price_inc_tax * $purchase_line->quantity_damage_returned;

                //Decrease quantity in variation location details
                if ($old_return_qty != $purchase_line->quantity_damage_returned) {
                    $this->productUtil->decreaseProductQuantity(
                        $purchase_line->product_id,
                        $purchase_line->variation_id,
                        $purchase->location_id,
            ...
            if (empty($request->input('ref_no'))) {
                //Update reference count
                $ref_count = $this->transactionUtil->setAndGetReferenceCount('purchase_damage_return');
                $return_transaction_data['ref_no'] = $this->transactionUtil->generateReferenceNumber('purchase_damage_return', $ref_count);
            }

            $return_transaction = Transaction::where('business_id', $business_id)
                ->where('type', 'purchase_damage_return')
                ->where('return_parent_id', $purchase->id)
                ->first();
            ...
            if (!empty($return_transaction)) {
                ...
                $return_transaction_data['business_id'] = $business_id;
                $return_transaction_data['location_id'] = $purchase->location_id;
                $return_transaction_data['type'] = 'purchase_damage_return';
                $return_transaction_data['status'] = 'final';
                $return_transaction_data['contact_id'] = $purchase->contact_id;
                $return_transaction_data['transaction_date'] = \Carbon::now();
                $return_transaction_data['created_by'] = request()->session()->get('user.id');
            ...
            $output = [
                'success' => 1,
                'msg' => __('lang_v1.damage_return_added_success'),
            ];
        ...
        return redirect('purchase-damage-return')->with('status', $output);
    }
     ...
     * @return \Illuminate\Http\Response
     */
    public function show_damage($id)
    {
        if (!auth()->user()->can('purchase.view')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        return view('purchase_return.show_damage')
            ->with(compact('purchase', 'purchase_taxes', 'activities'));
    }
     ...
     * @return \Illuminate\Http\Response
     */
    public function destroy_damage($id)
    {
        if (!auth()->user()->can('purchase.delete')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        try {
            if (request()->ajax()) {
                ...
                if (empty($purchase_return->return_parent_id)) {
                    ...
                    $updated_purchase_lines = $parent_purchase->purchase_lines;
                    foreach ($updated_purchase_lines as $purchase_line) {
                        $this->productUtil->updateProductQuantity($parent_purchase->location_id, $purchase_line->product_id, $purchase_line->variation_id, $purchase_line->quantity_damage_returned, 0, null, false);
                        $purchase_line->quantity_damage_returned = 0;
                        $purchase_line->save();
                    }
                }
